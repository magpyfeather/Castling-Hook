...
func _ready():
	...
	#connect("make_line", get_tree().get_root().get_node("Grapple"), "make_line")
	#connect("hide_line", get_tree().get_root().get_node("Grapple"), "hide_line")
...
# You get your grapple charge back when you land, indicated by the octagon around you.
	if active:
		...
		if Input.is_action_just_released("click"):
			...
			#grapple_ready = true
			grappling = false
		# A developer command. Would be a shame if it were to fall into the wrong hands!
...
func update_sound(new_volume):
	if Prefs.mute_sound == true:
		...
		$HitSound.volume_db = linear2db(new_volume)
